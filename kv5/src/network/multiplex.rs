...
mod tests {
    ...
    use super::*;
    use crate::{
        assert_res_ok, utils::DummyStream, CommandRequest, KvError, MemTable, ProstClientStream,
        ProstServerStream, Service, ServiceInner, Storage,
    };
    use anyhow::Result;
    ...
    pub async fn start_server_with<Store>(
        addr: &str,
        store: Store,
        f: impl Fn(TcpStream, Service) + Send + Sync + 'static,
        ...
        tokio::spawn(async move {
            loop {
                match listener.accept().await {
                    Ok((stream, _addr)) => f(stream, service.clone()),
    ...
    /// 创建 ymaux server
    pub async fn start_yamux_server<Store>(addr: &str, store: Store) -> Result<SocketAddr, KvError>
    where
        Store: Storage,
        Service: From<ServiceInner<Store>>,
    {
        ...
        start_server_with(addr, store, f).await
    }
    ...
    async fn yamux_ctrl_client_server_should_work() -> Result<()> {
        ...
        let addr = start_yamux_server("127.0.0.1:0", MemTable::new()).await?;
